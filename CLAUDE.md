...
- 依存はコンテナ内で入れる（`docker compose exec app npm install <pkg>`）。**ホストにも別の
  ...
  --ignore-scripts` でホストにも入れる（入れないとエディタにだけ型解決エラーが出る）。
...
- **三項演算子で JSX を出し分けない**（`eslint-rules/no-conditional-jsx.mjs`）。consequent / alternate の
  どちらかが JSX 要素・フラグメントなら違反（`cond ? <A/> : null` も、属性値の中の JSX も対象）。
  出しわけは**早期 return・変数への切り出し・論理積 `&&`**。文字列やクラス名など**短い値の三項は対象外**
  （`sending ? "送信中..." : "送信する"` は使ってよい）。

そのほか、レビューで見るもの:

- **テキストは `commons/Text.tsx` の `Text` 経由で描く**（`variant` + `tone`、`as` で要素を差し替え）。
  クラス文字列が要る箇所向けに `textStyles` / `toneStyles` も export している。足りなければ
  `textStyles` に1段追加し、**ページ側に arbitrary value を書かない**。
...
- preflight が `*` に `margin: 0; padding: 0` を当てるので **`p` / `h1`-`h6` / `ul` に `m-0` は書かない**。
  箇条書きは `commons/BulletList.tsx` を使う。
- クラス結合は `lib/cn.ts` の `cn()`（clsx + tailwind-merge、衝突は後勝ち）。カスタム字間
  （`tracking-heading` / `tracking-label`）は `extendTailwindMerge` に登録済み。
...
- ページに出すデータは `lib/` に集約し、ページは map して描画する。
...
- `GlassCard` の `span` / `start` は `lg:col-span-*` / `lg:col-start-*` なので **lg 以上でだけ効き、lg 未満は
  1カラム**。`hoverEffects={false}` でホバー演出を止める（遷移しないカード向け）。

### 日本語の文節改行（BudouX）

BudouX でビルド時に文節境界を求め、`<wbr>` として HTML に埋め込む。面は `commons/Text.tsx` の
**`phraseWrap`（`break-keep wrap-break-word`）**で、`keep-all` が日本語の任意改行を止め、`<wbr>` の
位置だけ改行を許す。`wrap-break-word` は1文節が行幅を超えたときの保険で、
**`wrap-anywhere`（`overflow-wrap: anywhere`）は使わない**（min-content 幅にも効いて列幅を変える）。
...
- **完成テキストの層と打ち込み中の層は両方とも文節改行にする**（片方だけだと折り返しが食い違って
  文字が跳ねる）。**打ち込み中の層は未入力ぶんの場所も `visibility:hidden` で確保する**
  （`withPhraseBreaks(lines, typedCount)`）。素直に切り出すと、行末の文節が「1〜2文字のうちは前の行に
  収まる → 伸びた瞬間に次の行へ飛ぶ」跳ね方をする。**ただし1文字も打つ前（＝SSG が出す HTML）は
  空にする** — 静的 HTML にテキストが2回入り `h1` の `textContent` が二重になる
  （`e2e/motion.spec.ts` が見張っている）。
...
- `cases.ts` の `brewCase` は Home / `/works` / `/works/brew` の3ページから参照する単一ソース。
- `skills.ts` の **`percent` は optional で、Home に載せるかどうかの分岐を兼ねる**。Home は `percent` を
  持つ項目だけの派生配列 `homeSkillGroups` を使い、**並べ替えはしない**（データ順＝バーの順）。
  構造の詳細は `README.md`。
- `home.ts` は文言（`heroCopy`）・文節（`heroPhrases`）・スケジュール（`heroTyping`）。**遅延は文字数から
  静的に算出する**ので、文言を書き換えれば追従する。
- `phrase.ts` は BudouX のパーサで**サーバー専用**。`polyhedra.ts` は頂点だけを持ち、**辺は頂点間の
  最小距離にあるペアから導出**する。
...
- **`GlassCard` の既定 padding（`p-4 sm:p-7` / `padding="lg"` は `p-5 sm:p-9`）はメニューパネルに
  波及しない**（`SiteNav` が `className` で `p-2.5 sm:p-7` に上書きしている）。逆にパネル側の `p-2.5` を
  変えるとロゴの位置がずれるので上のオフセット計算をやり直す。
- **Home ヒーローの高さは `h-svh`**。`100vh` は URL バーが隠れたときの高さなので、`h-screen` /
  ...
  一致するため e2e で検知できない。**
- **`/works/brew` の padding を変えたら `next/image` の `sizes` も直す**（hero モックが
  `calc(100vw - 94px)`、ワイド画像が `calc(100vw - 46px)`）。

### メニューパネル（sm 未満）

- **閉じる契機は `pathname` の effect とパネル内リンクの `onClick` の2つ**。effect だけだと現在地の
  リンクを踏んだとき `pathname` が変わらず閉じない。`onClick` だけだとブラウザバックで閉じない。
- **パネルの z（`z-40`）は nav（`z-50`）より下**。nav は `fixed` + `z-index` で stacking context を作るので、
  閉じるボタンに z を積んでも外側のパネルより手前には出せない。透明な nav がパネル上端のリンクを
  遮らないよう `max-sm:pointer-events-none` を当て、ロゴとハンバーガーにだけ `pointer-events-auto` を戻す。
- 開いている間は nav 側のロゴを `max-sm:invisible` で伏せ、パネル側のロゴが同じ位置に来るように
  オフセットを決めてある。**グリッドの `pr-24` は消さない**（右上に重なる × の線が `w-10`(40px) で、
  下回ると1行目のリンクと重なる）。
- 現在地の「+」は**絶対配置 + `aria-hidden`**（付けないとアクセシブルネームが「+works」になり、名前で
  引く e2e が落ちる）。SiteNav・`BackLink` のホバーの「+」も同じ理由で絶対配置 + `aria-hidden`。

### モーション

- `useRiseIn` は `transitionend`（`propertyName === "translate"`）でクラスを外す。**開始前のホバーでも
  `hover:-translate-y-0.5` が同じイベントを飛ばす**ので `started` で弾いている（弾かないと表示前の
  カードをホバーしただけで reveal が打ち切られる）。transition 対象は `transform` ではなく **`translate`**。
...
- **遷移先スクロールの受け渡しに `window.location.hash` を使わない**。App Router はルートごとに
  ハッシュ込みの正規 URL を保持しており、一度ハッシュ付きで遷移すると以後の普通の遷移でも復活する。
  `lib/scrollTarget.ts` の `sessionStorage` で一度きり読み捨て、**`ScrollToTarget` はルートレイアウトに
  1つだけ**置く（ページ側に置くと、着く前に別ページへ逸れた指定が残る）。

### 背景の正多面体（`commons/Wireframe.tsx`）

- **組み上げは CSS、自転は JS**と役割を分ける（触るプロパティが重ならないので競合しない）。
  組み上げは**マウントするまで `animation-play-state: paused`** にして、hydration 後に始まる
  Typewriter と起点を揃える。**尺（260ms / 420ms）は `globals.css` と `Wireframe.tsx` の
  `VERTEX_MS` / `EDGE_MS` の2箇所にある**ので、片方だけ変えると最後の辺が引き終わらない。
- **辺には `pathLength="1"` を置く**（自転で実長が毎フレーム変わるため、正規化しないと
  `stroke-dashoffset` の線引きが破綻する）。
- **`vector-effect="non-scaling-stroke"` は使わない**。dash の単位まで画面ピクセルになり `pathLength` の
  正規化が無視されるため、線引きが効かず**図形が最初から出来上がって見える**。ヘアラインは
  ResizeObserver で実サイズを測り `stroke-width` を user unit で逆算して保つ
  （`e2e/geometry.spec.ts` が設定と実測 1px の両方を見張っている）。
- **投影結果は小数4桁に丸める**。`Math.cos` / `Math.sin` はエンジンで最下位ビットが食い違うため、
  丸めないと SSR の属性値と WebKit の計算値が一致せず **hydration mismatch** になる。
- **伏せた姿は presentation attribute で持つ**（`stroke-dashoffset="1"` / `opacity="0"`。CSS の到着が
  遅れても完成形がちらつかないため）。アニメーションを止める場面では `globals.css` が素の値へ戻す
  ので、**インライン style にするとこの上書きが効かなくなる**。
...
- **「送信までの経過時間が短すぎたら弾く」判定は入れない**。クライアントが値を作れるので
  フォームを介さない POST には無力な一方、オートフィルで素早く送った実在の訪問者のメールを
  成功表示のまま捨ててしまう。
- Turnstile は常に `normal`(300px) で描き、外側の「置き場」の実幅 / 300 を内側の `scale` に入れて縮める
  （`origin-top-left`、倍率の上限は 1）。`transform` はレイアウトに影響しないので**外側 div に `scale` 後の
  高さを持たせる**。サイズは `render` 時にしか渡せないが、`scale` なら描き直さずに追従できる。

### E2E

- **`goto` 直後に1回だけスクロールするテストは hydration とレースする**。リスナーが付く前に
  スクロールが終わると以後 `scroll` が飛ばない。`e2e/motion.spec.ts` の `scrollUntilHidden` のように
  反応するまでやり直す。
- **`test.use({ reducedMotion: "reduce" })` はこの構成では反映されない**。`goto` の前に
  `page.emulateMedia({ reducedMotion: "reduce" })` を呼ぶ。
- **モバイル向けの spec は `e2e/responsive.spec.ts` に閉じる**（他の spec は 1280px 前提の期待値で
  書かれている）。デスクトップの2プロジェクトは `testIgnore`、モバイルの2プロジェクトは `testMatch`。
- 重ねた2層はグリッドアイテムで blockify されるため、外形を比べても内部の折り返しの違いは出ない。
  行位置は Range の矩形で比べる。

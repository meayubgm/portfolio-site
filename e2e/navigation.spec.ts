...
/**
 * ナビゲーション挙動:
 * - SiteNav のリンク遷移と active 判定（usePathname 由来の text-navy / text-slate 切り替え）
...
test.describe("SiteNav", () => {
  ...
  test("Home では home が active（text-navy）", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByRole("link", { name: "home" })).toHaveClass(/text-navy/);
    await expect(page.getByRole("link", { name: "works" })).toHaveClass(/text-slate/);
  });

  test("Works では works が active（text-navy）", async ({ page }) => {
    await page.goto("/works");
    await expect(page.getByRole("link", { name: "works" })).toHaveClass(/text-navy/);
    await expect(page.getByRole("link", { name: "home" })).toHaveClass(/text-slate/);
  });
});
